...
void loop() {
  ...
  int reducedPower = avgPower;  
  if(d < 20){
    reducedPower = (int)((float)reducedPower * 0.7);
  }
  ...
  int delta = (rightSteps - leftSteps) * 5;

  int rightPower = reducedPower - delta;
  int leftPower = reducedPower + delta;
  ...
  if(d > 15){
    forwardLeftRight(leftPower, rightPower);
  }
  else if(d < 10){
    backwards(reducedPower);
  }
  else{
    stopCar();
  }
...
void forwardLeftRight(int powerLeft, int powerRight){
  digitalWrite(motorA_pin, HIGH);
  digitalWrite(motorB_pin, LOW);
  digitalWrite(motorC_pin, HIGH);
  digitalWrite(motorD_pin, LOW);
  analogWrite(powerLeft_pin, powerLeft);
  analogWrite(powerRight_pin, powerRight);
}
